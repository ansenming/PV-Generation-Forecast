...
data_raw = np.zeros((33858,7),dtype = np.float32)
data_ripe = np.zeros((2208,7), dtype = np.float32)

with open('./fall.csv', 'r') as file:
    rows = file.readlines()
    i = 0
    for row in rows:
        '''
...
#--------------------------------------------------
#最後兩天當testing data
x_train = np.zeros((2112,6),dtype = np.float32)
y_train = np.zeros((2112,),dtype = np.float32)
x_test = np.zeros((96,6), dtype = np.float32)
y_test = np.zeros((96,), dtype = np.float32)

x_train = data_ripe[:2112,0:6]
y_train = data_ripe[:2112, 6]
x_test = data_ripe[2112:, 0:6]
y_test = data_ripe[2112:, 6]
...
#--------------------------------------------------
#RandomForest
rf = RandomForestRegressor(n_estimators = 600, random_state = 42, max_depth = 7)
